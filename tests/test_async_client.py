...
async def test_async_create_and_paginate():
    ...
    async with AsyncMyStarsClient(API_KEY, transport=transport, retry=RetryPolicy(base_delay=0.001, jitter=False), sleep=_noop_sleep, rand=lambda: 0.0) as client:
        res = await client.create_order(type="stars", recipient="durov", quantity=100, payment_currency="ton")
...
@pytest.mark.asyncio
async def test_async_create_order_rejects_an_unknown_payment_currency_before_any_request():
    from mystars_faas import MyStarsValidationError

    transport, calls = _transport([{"status": 201, "json": CREATED}])
    async with AsyncMyStarsClient(API_KEY, transport=transport) as client:
        with pytest.raises(MyStarsValidationError):
            await client.create_order(type="stars", recipient="durov", quantity=100, payment_currency="usdt")  # type: ignore[arg-type]
    assert calls == []


@pytest.mark.asyncio
async def test_async_payment_currency_is_a_required_argument():
    transport, calls = _transport([])
    async with AsyncMyStarsClient(API_KEY, transport=transport) as client:
        with pytest.raises(TypeError, match="payment_currency"):
            await client.get_pricing(type="stars", quantity=100)  # type: ignore[call-arg]
        with pytest.raises(TypeError, match="payment_currency"):
            await client.get_pricing_batch(quantities=[50])  # type: ignore[call-arg]
        with pytest.raises(TypeError, match="payment_currency"):
            await client.create_order(type="stars", recipient="durov", quantity=100)  # type: ignore[call-arg]
    assert calls == []


@pytest.mark.asyncio
async def test_async_pricing_validates_payment_currency():
    from mystars_faas import MyStarsValidationError

    transport, calls = _transport([])
    async with AsyncMyStarsClient(API_KEY, transport=transport) as client:
        with pytest.raises(MyStarsValidationError):
            await client.get_pricing(type="stars", quantity=100, payment_currency="usdt")  # type: ignore[arg-type]
        with pytest.raises(MyStarsValidationError):
            await client.get_pricing_batch(quantities=[50], payment_currency="usdt")  # type: ignore[arg-type]
    assert calls == []