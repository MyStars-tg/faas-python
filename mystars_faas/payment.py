...
JETTON_TRANSFER_GAS_NANO = "50000000"  # 0.05 GRAM

# Same grammar as the TS SDK's DECIMAL_RE (`/^\d+(\.\d+)?$/`): an unsigned plain
# decimal only — rejects a sign (a payment amount is never negative), scientific
# notation (`1e3`), bare dots, and surrounding whitespace, so the two SDKs
...
def to_nano(amount: str | Decimal | int) -> int:
    """Convert a decimal GRAM amount to integer nanoTON (×1e9).

    Args:
        amount: A GRAM amount as a string, ``Decimal``, or int. For a **string** input,
            a sign (``-``/``+``), scientific notation, and whitespace are rejected (unsigned
            plain decimal only). A ``Decimal``/int is first normalised to fixed-point, so a
            ``Decimal`` in scientific notation such as ``Decimal('1E-9')`` is accepted; a
            negative one is rejected.

    Returns:
        The amount in nanoTON (1 GRAM = 1,000,000,000 nanoTON).

    Raises:
        MyStarsValidationError: If ``amount`` is negative or not a plain decimal.
    """
...
def to_micro(amount: str | Decimal | int) -> int:
    ...
    Args:
        amount: A USDT amount as a string, ``Decimal``, or int. A **string** must be an
            unsigned plain decimal (no sign, no scientific notation); a ``Decimal``/int is
            normalised to fixed-point first, so ``Decimal('1E-9')`` is accepted and a negative
            value is rejected.
...
class _Rail:
    """What a currency code means on-chain — the one table every builder branches on."""
...
_RAILS = {
    "ton": _Rail(asset="GRAM", amount_units="ton", decimals=9, transfer="native"),
    "usdt_ton": _Rail(asset="USDT", amount_units="usdt", decimals=6, transfer="jetton"),
}


# The payment-block fields that restate the currency; each must agree with it when present.
_RAIL_FIELDS = ("amount_units", "asset", "decimals", "transfer")


@dataclass(frozen=True)
class _VerifiedPayment:
    """A payment instruction that passed :func:`_verify_payment`."""

    transfer: str  # "native" (a plain GRAM transfer) or "jetton" (a TEP-74 USDT transfer)
    units: int  # the positive amount in nanoTON / micro-USDT


def _verify_payment(payment: PaymentInstruction) -> _VerifiedPayment:
    """The consistency check every builder runs before producing anything payable.

    A payment block names its asset twice: by ``currency``, and by the fields that restate it
    (``amount_units``, and from contract v1.15.0 ``asset``, ``decimals``, ``transfer``). Sending
    the wrong asset is not recoverable by the payer, so this refuses to guess: an unknown
    ``currency``, or a restating field that disagrees with it, raises instead of falling through
    to a default rail. The amount is stated twice as well from v1.15.0, and
    ``amount_smallest_unit`` must equal ``amount``. Mirrors the TypeScript SDK's
    ``verifyPaymentInstruction``.

    Args:
        payment: The order's :class:`~mystars_faas.PaymentInstruction`.

    Returns:
        The transfer kind and the positive smallest-unit amount.

    Raises:
        MyStarsValidationError: On an unknown currency, a contradicting field, a malformed, zero
            or negative amount, or an ``amount_smallest_unit`` that is not exactly ``amount`` in the
            asset's smallest unit.
    """
    rail = _RAILS.get(payment.currency)
    if rail is None:
        raise MyStarsValidationError(
            f'unknown payment.currency "{payment.currency}" — expected "ton" or "usdt_ton"; refusing to guess the asset'
        )
    for field_name in _RAIL_FIELDS:
        declared = getattr(payment, field_name)
        expected = getattr(rail, field_name)
        if declared is not None and declared != expected:
            raise MyStarsValidationError(
                f'payment.{field_name} "{declared}" contradicts payment.currency "{payment.currency}" (expected "{expected}")'
            )
    units = _to_units(payment.amount, rail.decimals)
    if units <= 0:
        raise MyStarsValidationError(f'payment amount must be positive, got "{payment.amount}"')
    # The server sends ``amount_smallest_unit`` as the stored integer and renders ``amount`` from it
    # without rounding, so the two agree exactly — any difference is a corrupt block, not rounding.
    declared_units = payment.amount_smallest_unit
    if declared_units is not None and declared_units != str(units):
        raise MyStarsValidationError(
            f'payment.amount_smallest_unit "{declared_units}" contradicts payment.amount "{payment.amount}" (expected "{units}")'
        )
    return _VerifiedPayment(transfer=rail.transfer, units=units)


# ─── deeplink + payment request ───────────────────────────────────────────────

def build_ton_deeplink(payment: PaymentInstruction) -> str:
    """Build a ``ton://transfer/...`` deeplink for a GRAM payment instruction.
    ...
    Raises:
        MyStarsValidationError: If the payment block is inconsistent (unknown currency, a field
            contradicting it, a non-positive amount), ``payment.currency`` is not ``ton`` (USDT
            needs a jetton transfer), or ``pay_to_address`` / ``memo`` is missing.
    """
    verified = _verify_payment(payment)
    if verified.transfer != "native":
...
class TonConnectMessage:
    ...
    Attributes:
        address: The destination address (the pay-to address for GRAM; the payer's own jetton
            wallet for USDT).
        amount: The attached GRAM value in nanoTON, as a string.
        payload: The base64 BoC body (op-0 comment for GRAM; jetton-transfer body for USDT).
    """
...
class PaymentRequest:
    ...
    Attributes:
        ...
        ton_deeplink: A ``ton://`` deeplink (GRAM only).
        tonkeeper_link: A Tonkeeper universal link (GRAM only).
        qr_payload: A string to render as a QR code (GRAM only).
...
def build_ton_connect_messages(
    ...
    Raises:
        MyStarsValidationError: If ``pay_to_address`` / ``memo`` is missing, the payment block is
            inconsistent (unknown currency, a field contradicting it, a non-positive amount), or
            (for USDT) the payer's ``sender_address`` / ``jetton_wallet_address`` is not supplied.
    """
    pay_to = _require(payment.pay_to_address, "pay_to_address")
    memo = _require(payment.memo, "memo")
    verified = _verify_payment(payment)
    if verified.transfer == "native":
        return [TonConnectMessage(address=pay_to, amount=str(verified.units), payload=build_comment_payload(memo))]
    ...
    payload = build_jetton_transfer_payload(verified.units, pay_to, sender_address, memo)
...
def build_payment_request(
    ...
    Raises:
        MyStarsValidationError: If ``pay_to_address`` / ``memo`` is missing, or the payment block is
            inconsistent (unknown currency, a field contradicting it, a malformed or non-positive
            amount).
    """
    pay_to = _require(payment.pay_to_address, "pay_to_address")
    memo = _require(payment.memo, "memo")
