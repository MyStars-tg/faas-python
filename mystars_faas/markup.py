...
def ceil_ton_to_4dp(ton: Decimal | str | float | int) -> Decimal:
    """Ceil a GRAM amount to the 0.0001-GRAM grid (snap to nanoTON, then ceil)."""
...
class RetailQuote:
    ...
    All money fields are formatted decimal strings (USD to 2 dp, GRAM trimmed to ≤4 dp).
...
def apply_retail_markup(
    ...
    *,
    ...
    Uses the server's exact two-stage cent-ceil (USD) / 0.0001-grid-ceil (GRAM) so the price you
