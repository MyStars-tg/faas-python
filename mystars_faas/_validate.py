...
PaymentCurrency = Literal["ton", "usdt_ton"]
"""An accepted ``payment_currency``: ``ton`` (the native GRAM coin) or ``usdt_ton`` (USDT on the TON network)."""

PAYMENT_CURRENCIES: tuple[str, ...] = get_args(PaymentCurrency)
...
def assert_payment_currency(currency: str) -> None:
    """Validate a ``payment_currency`` code.

    Args:
        currency: The code to check.

    Raises:
        MyStarsValidationError: If it is not one of ``PAYMENT_CURRENCIES`` (``"ton"`` or ``"usdt_ton"``).
    """
    if currency not in PAYMENT_CURRENCIES:
        raise MyStarsValidationError(f'payment_currency must be "ton" or "usdt_ton", got {currency!r}')
