...
from ._validate import (
    PaymentCurrency,
    assert_order_type,
    assert_payment_currency,
    assert_premium_months,
    assert_stars_quantity,
    canonical_username,
)
...
class MyStarsClient:
    ...
    def get_pricing(
        self, *, type: str, quantity: int | None = None, months: int | None = None, payment_currency: PaymentCurrency
    ) -> PricingQuote:
        ...
        Args:
            ...
            payment_currency: Required — ``"ton"`` (the native GRAM coin) or ``"usdt_ton"`` (USDT).
        ...
        assert_payment_currency(payment_currency)
    ...
    def get_pricing_batch(self, *, quantities: list[int], payment_currency: PaymentCurrency) -> PricingQuoteBatch:
    ...
    def check_recipient(self, username: str, *, type: str, months: int | None = None) -> RecipientCheck:
        ...
        ``eligible`` alone is not the whole answer: read ``indeterminate`` too. When it is
        ``True`` the check failed open without reaching a verdict, so ``eligible`` is a
        permissive default rather than a measurement — safe to order against, but never
        something to show a buyer as a confirmed recipient.
        ...
        Returns:
            ...
            ``indeterminate``, ``recipient_name``, ``reason``, ``telegram_message``).
